...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	q := Queries{db: db}
	var err error
	if q.accumulateActiveTimeStmt, err = db.PrepareContext(ctx, accumulateActiveTime); err != nil {
		return nil, fmt.Errorf("error preparing query AccumulateActiveTime: %w", err)
	}
...
func (q *Queries) Close() error {
	var err error
	if q.accumulateActiveTimeStmt != nil {
		if cerr := q.accumulateActiveTimeStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing accumulateActiveTimeStmt: %w", cerr)
		}
	}
	...
	if q.getGoalBySessionIDStmt != nil {
		if cerr := q.getGoalBySessionIDStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getGoalBySessionIDStmt: %w", cerr)
		}
	}
	...
	if q.listMessagesBySessionFromSummaryStmt != nil {
		if cerr := q.listMessagesBySessionFromSummaryStmt.Close(); cerr != nil {
